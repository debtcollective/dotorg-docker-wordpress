...
if [ "$ENVIRONMENT" == "production" ]; then
	...
	cp -rf /tmp/wp-action-network-events-main/* /usr/src/wordpress/wp-content/plugins/wp-action-network-events
	...
	unzip /tmp/wp-site.zip -d /tmp/
	unzip /tmp/wp-action.zip -d /tmp/
	cp -rf /tmp/wp-action-network-events-staging/* /usr/src/wordpress/wp-content/plugins/wp-action-network-events
	...
	unzip /tmp/wp-site.zip -d /tmp/
	unzip /tmp/wp-action.zip -d /tmp/
	cp -rf /tmp/wp-action-network-events-development/* /usr/src/wordpress/wp-content/plugins/wp-action-network-events
